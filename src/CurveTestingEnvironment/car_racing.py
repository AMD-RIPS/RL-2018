# This version of the Car Racing environment has the capability to characterize curves in the race track.
...
class CarRacing(gym.Env):
    ...
    def _create_track(self):
        ...
        self.track = track
        self.characterize_curves()
        return True


    def characterize_curves(self):
        # find curves
        curves = []
        i = 0
        while i < len(self.road):
            curve = []
            alpha1, beta1, x1, y1 = self.track[i]
            alpha2, beta2, x2, y2 = self.track[i-1]

            # curve found
            while i < len(self.road) and abs(beta1 - beta2) > 0.05:
                alpha1, beta1, x1, y1 = self.track[i]
                alpha2, beta2, x2, y2 = self.track[i-1]  
                curve.append([beta1, i])
                i += 1
        ...
        for curve in curves:
            ...
            for i in range(1, len(curve)):
                index = curve[i][1]
                diff = curve[i][0] - curve[i-1][0] # difference in beta
                if (diff) > 0:
                    left += 1
                else:
                    right += 1
                total.append(abs(diff))

            # Left Curve, Right Curve, or S-shape
            if left < 2:
                curve.append("R")
            elif right < 2:
                curve.append("L")
            ...
            # Sharp or Shallow Curve
            gradient = sum(heapq.nlargest(5, total))/5
            steepness = bisect.bisect_left([0,.10,.20,.25,.30,.3101], gradient)
            curve.append(steepness)
        ...
        #         index = curve[i][1]
        #         self.road_poly[index] = (self.road_poly[index][0], color)

        # save curve data
        self.curves = []
        for curve in curves:
            tiles = [curve[i][1] for i in range(len(curve) - 2)]
            self.curves.append([tiles, curve[-2], curve[-1]])

    def test_curves(self):
        result = []
        for curve in self.curves:
            tiles = curve[0]
            tiles_visited = sum([self.road[tile].road_visited for tile in tiles])
            result.append([tiles_visited, len(tiles), curve[-2], curve[-1]])
        return result
