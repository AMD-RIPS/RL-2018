...
class DQN_Agent:
    ...
    def train(self, test_frequency, test_episodes):
        while self.sess.run(self.episode) < self.training_metadata.num_episodes:
            ...
            if (episode % test_frequency == 0) and (episode != 0):
                ...
                self.saver.save(self.sess, self.checkpoint_path + '/data.chkp', global_step=self.training_metadata.episode)
    ...
    def test(self, num_test_episodes, visualize, log=False, test_curves=False):        
        ...
        for episode in range(num_test_episodes):
            ...
            if test_curves:
                self.env.test_curves()
        if test_curves:
            self.env.analyze_curves(self.log_path + '/curve_data.txt')
        if log:
